...
# Initialize exactly like the authors
...
# Sort so left group (lower X centroid) is mask0
...
# rd = right group (mask1), ld = left group (mask0)
# -Z = up (away from table), +Z = down (toward table)
# +X = right, -X = left
motions = [
    ('Both lift',       [0,0,-0.001],  [0,0,-0.001],  100),
    ('Pull apart',      [0.001,0,0],   [-0.001,0,0],  100),
    ('Fold R over L',   [-0.002,0,-0.001], [0,0,0],   160),
    ('Fold L over R',   [0,0,0],       [0.002,0,-0.001], 160),
    ('Both drag fwd',   [0,0.001,0],   [0,0.001,0],   100),
    ('Twist',           [0,0.001,0],   [0,-0.001,0],  100),
    ('Push together',   [-0.001,0,0],  [0.001,0,0],   100),
    ('Lift + stretch',  [0.0008,0,-0.0006], [-0.0008,0,-0.0006], 100),
]
...
for motion_name, rd, ld, num_steps in motions:
    ...
    x0 = wp.to_torch(sim.wp_states[0].wp_x, requires_grad=False)
...
out_dir = 'experiments/trajectory_01/dual_gripper_v15'
