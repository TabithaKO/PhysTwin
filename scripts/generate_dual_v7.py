import pickle, json, glob, torch, numpy as np, warp as wp, cv2, os
...
# KMeans split into 2 control groups
...
# === Motion definitions ===
# spring_Y_override=None means use learned, otherwise override
# settle/stabilize params per motion
motions = [
    # (name, rd, ld, steps, spring_Y_override, settle_steps, stab_steps)
    ('Both lift',       [0,0,-0.0003], [0,0,-0.0003], 500, 1000, 200, 200),
    ('Pull apart',      [0.001,0,0],   [-0.001,0,0],  100, None, 0, 0),
    ('Fold R over L',   [-0.002,0,-0.001], [0,0,0],   160, None, 0, 0),
    ('Fold L over R',   [0,0,0],       [0.002,0,-0.001], 160, None, 0, 0),
    ('Both drag fwd',   [0,0.001,0],   [0,0.001,0],   100, None, 0, 0),
    ('Twist',           [0,0.001,0],   [0,-0.001,0],  100, None, 0, 0),
    ('Push together',   [-0.001,0,0],  [0.001,0,0],   100, None, 0, 0),
    ('Lift + stretch',  [0.0008,0,-0.0006], [-0.0008,0,-0.0006], 100, None, 0, 0),
]

all_results = []
for motion_name, rd, ld, num_steps, sy_override, settle_n, stab_n in motions:
    ...
    if sy_override is not None:
        ...
        print(f'  Spring Y override: {sy_override}')
    else:
    ...
    if stab_n > 0:
        ...
        print(f'  Stabilizing {stab_n} steps (0.1 damping)...')
...
out_dir = 'experiments/trajectory_01/dual_gripper_v7'
